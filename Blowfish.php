...
define("CBC", 1);
     ...
     echo ("<br> plainteks : &nbsp");
     echo $plain;
	 echo ("<br> Cipherteks: &nbsp");	 
	 echo $cipher; */
...
class Blowfish {
   ...
   function Blowfish($key) {
      $this->kunci($key);
	  //print_r("<br> BLowfish Awal//isi dari this baris 18 = &nbsp;");
	 // print_r($this);
   }
   ...
   function Str2Long($data) {
	    // print_r("<br> Str2Long//isi dari data= &nbsp;");
	    // print_r($data);
       $tmp = unpack('N*', $data);
	    // print_r("<br> Str2Long//isi dari tmp= &nbsp;");
	   //  print_r($tmp);
       $data_long = array();
	    // print_r("<br> Str2Long//isi dari data long = &nbsp;");
	    // print_r($data_long);
       $j = 0;
   ...
   function kunci($key) {
      ...
      for($i=0;$i<count($F2);$i+=2) {
         $LR = $this->enchiper($LR[0],$LR[1]);
         $this->F2[$i] = $LR[0];
         $this->F2[$i+1] = $LR[1];
      }

      for($i=0;$i<count($F3);$i+=2) {
         $LR = $this->enchiper($LR[0],$LR[1]);
         $this->F3[$i] = $LR[0];
         $this->F3[$i+1] = $LR[1];
      }
   ...
   function enkripsi($text) {
      ...
      for($i = 0; $i<count($text); $i+=2) {
         if(CBC == 1) {
            ...
            //XOR dengan ciphertext sebelumnya
            $text[$i] ^= $cipher[$a-1][0];
            $text[$i+1] ^= $cipher[$a-1][1];
         }

         $cipher[] = $this->enchiper($text[$i],$text[$i+1]);
         $a++;
      }
   ...
   function dekripsi($text) {
      ...
      for($i; $i<count($cipher); $i+=2) {
         $return = $this->decipher($cipher[$i],$cipher[$i+1]);
